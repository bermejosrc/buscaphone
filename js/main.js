...
function cargarContenidoInicio() {  
    ...
    document.getElementById("cajaBusqueda").addEventListener("keyup", function() {
        if (event.keyCode === 13) {
            location.href=basePath+'/html/explorar.html?buscar=' + cajaBusqueda.value;
          }
    });
}
...
function cambiarImagen() {
    ...
    var imagenes = [
        basePath+"/img/paseImagenes/imagen1.png",
        basePath+"/img/paseImagenes/imagen2.png",
        basePath+"/img/paseImagenes/imagen3.png"
    ]
...
function crearParrilla(elementos, parrilla) {
    ...
    for(var i = 0; i < elementos.length; i++) {
        ...
        parrillaEnlace.href = basePath+"/html/explorar.html?ref=" + elementos[i][0];

        parrillaImg = document.createElement("img");
        parrillaImg.src = basePath+elementos[i][2];
    ...
    if(resultados.length == 0) {
        ...
        for(var i = 0; i < resultados.length; i++) {
            ...
            if(resultados[i][1] == "producto") {
                resultado.href = basePath+"/html/explorar.html?ref=" + resultados[i][2];
            } else if(resultados[i][1] == "tienda") {
                resultado.href = basePath+"/html/explorar.html?tienda=" + resultados[i][2];
            } else if(resultados[i][1] == "categoria") {
                if(resultados[i][2] == "1") {
                    resultado.href = basePath+"/html/explorar.html?cat=1,2";
                } else if(resultados[i][2] == "2") {
                    resultado.href = basePath+"/html/explorar.html?cat=3,4";
                } else if(resultados[i][2] == "3") {
                    resultado.href = basePath+"/html/explorar.html?cat=5,6,7,8";
                } else if(resultados[i][2] == "4") {
                    resultado.href = basePath+"/html/explorar.html?cat=9";
                }
            } else if(resultados[i][1] == "subcategoria") {
                resultado.href = basePath+"/html/explorar.html?cat=" + resultados[i][2];
            }
...
function enviarFormulario() {
    ...
    peticion.open("GET",basePath+"/php/formularios.php?consulta=" + consulta, true);
    peticion.send();
}
...
function getElementos(accion, consulta) {
    ...
    console.log("INFO> Se va a consultar al servidor: " + consulta);
    peticion.open("GET",basePath+"/php/productos.php?consulta=" + consulta, true);
    peticion.send();
}